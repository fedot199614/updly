...
import { SiteType, ExtractionResult } from "@/backend/services/extract/exctract.types.js";

export const processPage = async (
  page: any,
  forceType?: SiteType,
): Promise<{ html: string } & ExtractionResult & { hash: string }> => {

  const { html } = await fetchSmart(page);

  const result = extract(html, page.url, forceType);

  const hash = crypto
    .createHash("md5")
    .update(result.units.map((u) => u.hash).join("|"))
    .digest("hex");

  return { html, ...result, hash };
};